...
const nftCards = [
  { name: "Cosmic Drift #042", collection: "CosmicPunks", price: "0.85 ETH", rarity: "Rare", gradient: "from-violet-600 via-fuchsia-500 to-pink-500", emoji: "🌌" },
  { name: "Neon Owl #117", collection: "NightOwls", price: "1.20 ETH", rarity: "Epic", gradient: "from-cyan-500 via-blue-500 to-indigo-600", emoji: "🦉" },
  { name: "Rusted Titan #009", collection: "MetalHeads", price: "2.40 ETH", rarity: "Legendary", gradient: "from-amber-500 via-orange-500 to-red-600", emoji: "⚙️" },
  { name: "Bloom Spirit #331", collection: "GardenDAO", price: "0.45 ETH", rarity: "Uncommon", gradient: "from-emerald-500 via-teal-500 to-cyan-600", emoji: "🌸" },
  { name: "Dark Matter #007", collection: "VoidSeries", price: "3.10 ETH", rarity: "Legendary", gradient: "from-gray-600 via-slate-700 to-zinc-800", emoji: "🕳️" },
  { name: "Solar Flare #228", collection: "AstroPass", price: "0.62 ETH", rarity: "Rare", gradient: "from-yellow-400 via-orange-400 to-red-500", emoji: "☀️" },
];
...
function NFTCarousel3D() {
  ...
  // radius so cards don't overlap: card width ~200, circumference = count*220
  ...
  return (
    <div style={{ background: "#252526", border: `1px solid ${C.border}`, borderRadius: 4 }} className="p-4">
      ...
      <div className="relative flex items-center justify-center" style={{ height: 260, perspective: 900 }}>
        ...
        >
          {nftCards.map((card, i) => {
            ...
            return (
              ...
              >
                {/* Card gradient bg */}
                <div className={`w-full h-full bg-gradient-to-br ${card.gradient} flex flex-col justify-between p-3`}
                  style={{ background: undefined }}>
                  <div
                    className="w-full flex-1 flex items-center justify-center rounded-md mb-2"
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 4, padding: "6px 8px", zIndex: 10,
          }}
      ...
      {/* Active card detail */}
      <div style={{ marginTop: 12, padding: "8px 10px", background: "#1e1e1e", borderRadius: 4, border: `1px solid ${C.border}`, fontFamily: "monospace", fontSize: 11 }}>
  ...
  return (
    <div className="grid grid-cols-4 gap-3">
      {web3kpis.map((k) => {
        const Icon = k.icon;
        return (
          <div key={k.label} style={{ background: "#252526", border: `1px solid ${C.border}`, borderRadius: 4 }} className="p-3 relative overflow-hidden">
...
function CMSKPIs() {
  return (
    <div className="grid grid-cols-4 gap-3">
      {kpis.map((k) => (
        <div key={k.label} style={{ background: "#252526", border: `1px solid ${C.border}`, borderRadius: 4 }} className="p-3">
    ...
    <div style={{ background: "#252526", border: `1px solid ${C.border}`, borderRadius: 4 }} className="p-4">
      ...
      <div className="space-y-2 text-xs font-mono">
        {contractStatus.map((c, i) => (
          <div key={i} className="flex items-center gap-2 p-2" style={{ background: "#1e1e1e", borderRadius: 3 }}>
  ...
  return (
    <div style={{ background: "#252526", border: `1px solid ${C.border}`, borderRadius: 4 }} className="p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {levels.map((l) => (
          <div key={l.label} style={{ background: "#1e1e1e", border: `1px solid ${C.border}`, borderRadius: 3 }} className="p-2 text-xs font-mono">
    ...
    <div style={{ background: C.activity, borderRight: `1px solid ${C.border}`, width: 48 }} className="flex flex-col items-center py-2 shrink-0">
      {icons.map((Icon, i) => (
        <button key={i} style={{ color: i === 0 || i === 3 ? C.bright : C.dim }} className="w-12 h-12 flex items-center justify-center relative transition-colors hover:text-white">
          ...
          {i === 0 && <div style={{ background: C.bright, width: 2, height: 24, position: "absolute", left: 0, borderRadius: "0 2px 2px 0" }} />}
          {i === 3 && <div style={{ background: C.purple, width: 2, height: 24, position: "absolute", left: 0, borderRadius: "0 2px 2px 0" }} />}
        </button>
      ))}
...
export function Dashboard() {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: "'Segoe UI', system-ui, sans-serif", minHeight: "100vh" }} className="flex flex-col">
      {/* Title bar */}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div style={{ background: C.bg, borderBottom: `1px solid ${C.border}`, height: 24, fontSize: 12, color: C.dim }} className="flex items-center px-4 gap-1 shrink-0">
            <span>cms-web3</span><ChevronRight className="w-3 h-3" /><span>src</span><ChevronRight className="w-3 h-3" />
          ...
          <div style={{ background: C.bg, borderTop: `1px solid ${C.border}`, height: 100 }} className="shrink-0">
            <div style={{ background: "#252526", borderBottom: `1px solid ${C.border}`, height: 28, fontSize: 12 }} className="flex items-center">
              {["PROBLEMS (3)", "OUTPUT", "TERMINAL", "WEB3 CONSOLE"].map((t, i) => (
                <button key={t} style={{ color: i === 3 ? C.cyan : i === 0 ? C.bright : C.dim, borderBottom: (i === 0 || i === 3) ? `1px solid ${i === 3 ? C.cyan : C.accent}` : "1px solid transparent", padding: "0 16px", height: 28, fontSize: 12 }} className="hover:text-white transition-colors">{t}</button>
              ))}
              <div className="flex-1" />
